...
function getModelsDir(modelDir) {
  if (modelDir) {
    return path.dirname(path.resolve(modelDir));
  }
...
function getModelDir(modelDir) {
  return modelDir ? path.resolve(modelDir) : path.join(getModelsDir(), MODEL_SUBDIR);
}
...
function checkModels(modelDir) {
  const resolvedModelDir = getModelDir(modelDir);
  const files = ['encoder.int8.onnx', 'decoder.int8.onnx', 'tokens.txt'];

  for (const file of files) {
    const fullPath = path.join(resolvedModelDir, file);
 ...
 * 初始化 ASR 引擎
 */
async function initASR(modelDir) {
  if (recognizer) {
    // 已初始化，重置stream即可
  ...
  const config = {
    ...
    modelConfig: {
      paraformer: {
        encoder: path.join(resolvedModelDir, 'encoder.int8.onnx'),
        decoder: path.join(resolvedModelDir, 'decoder.int8.onnx'),
      },
      tokens: path.join(resolvedModelDir, 'tokens.txt'),
      numThreads: 2,
      provider: 'cpu',
      debug: false
    },
...
module.exports = { initASR, feedAudio, stopRecognition, checkModels, getModelsDir, getModelDir };
