 ...
 * @returns {string} 报告文本
 */
async function sendReport(fullText, stats, settings, customPrompt, context = null) {
  const config = getProviderConfig(settings);
  const prompt = getReportPrompt(fullText, stats, customPrompt, context);
  ...
  const result = await callAPI(config.endpoint, config.apiKey, config.model, messages, 3600);
  return result;
}
